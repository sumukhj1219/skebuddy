...
const CodeBlock = () => {
  ...
  const formattedJsx = useMemo(
    () => formatHtmlToJsx(document.getElementById(canvasTemplate.htmlId)?.innerHTML || ""),
  ...
  return (
    <div className="flex flex-col h-full rounded-lg border bg-background min-h-0 ">
      <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/40 shrink-0">
        <span className="text-sm font-mono text-secondary">SkeletonLayout.tsx</span>
      ...
      <div
        className="flex-1 overflow-auto min-h-0 bg-sidebar text-sm font-mono [&_pre]:m-0 [&_pre]:p-4 [&_pre]:box-border"
